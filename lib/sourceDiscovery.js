...
async function discoverFromGitHubTags(version, candidate, options) {
    ...
    const url = `https://api.github.com/repos/${encodeURIComponent(candidate.owner)}/${encodeURIComponent(candidate.name)}/tags`;
    try {
    ...
    for (let i = 0; i < tags.length; i++) {
      const tag = tags[i];
      if (tag.name === version || tag.name === `v${version}`)
        return { url: tag.commit.url, revision: tag.commit.sha };
    }
