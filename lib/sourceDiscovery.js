...
async function discoverFromGitHubRefs(version, candidate, options) {
  ...
  try {
    ...
    if (get(options, 'logger.error')) options.logger.error(`discoverFromGitHubRefs: ${url}`, error)
  }
}
...
async function discoverFromGitHubTags(version, candidate, options) {
  ...
  try {
    ...
    if (get(options, 'logger.error')) options.logger.error(`discoverFromGitHubTags ${url}`, error)
    return null
  }
}
