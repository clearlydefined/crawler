...
function resolveGitHubLocations(locations) {
  const result = locations
    .map(location => {
      const parsedUrl = location ? parseGitHubUrl(location) : null
      return parsedUrl?.owner && parsedUrl.name ? parsedUrl : null
    })
    .filter(e => e)
...
async function _discoverFromGitHubRefs(version, candidate, options) {
  const headers = {
  ...
  try {
    const refs = await ghrequestor.getAll(url, {
      ...
      maxAttempts: 3,
      retryDelay: 250,
      // TODO This is the default strategy in axios retry. We ghrequestor currently depends on requestretry, but we will replace it with axios retry soon.
    ...
    if (get(options, 'logger.error')) {
      options.logger.error(`discoverFromGitHubRefs: ${url}`, error)
    }
  }
}

async function discoverFromGitHubTags(version, candidate, options) {
  const url = `https://github.com/${encodeURIComponent(candidate.owner)}/${encodeURIComponent(candidate.name)}.git`
  try {
    const repo = geit(url)
    ...
    return (
      ...
      refs[`refs/tags/${version}`] ||
      refs[`refs/tags/v${version}`] ||
      refs[`refs/tags/rel/v${version}`] ||
      refs[`refs/tags/v.${version}`] ||
      refs[`refs/tags/${version}.0`] || // Improve NuGet source discovery
      refs[`refs/tags/v${version}.0`]
    )
  } catch (error) {
    if (get(options, 'logger.error')) {
      options.logger.error(`discoverFromGitHubTags ${url}`, error)
    }
    return null
  }
}
...
function getProvider(location) {
  const lowerUrl = location?.href ? location.href.toLowerCase() : ''
