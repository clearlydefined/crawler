      ...
      glob(
        ...
        (error, files) => {
          ...
          Promise.all(files.map(async file => {
            ...
            const licenses = await BaseHandler.detectLicenses(filePath)
            document._attachments.push({ path: file, token, attachment, licenses })
            return { path: file, token, licenses }
  ...
  static detectLicenses(filePath) {
    return new Promise((resolve, reject) => {
      exec(`licensee detect ${filePath} --json`, (error, stdout) => {
        ...
        resolve(result.licenses.map(x => x.spdx_id))
      })
    })
  }
