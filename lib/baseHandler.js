...
class BaseHandler {
  ...
  static async addInterestingFiles(document, location, folder = '') {
    ...
    document.interestingFiles = document._attachments.map(entry => {
      return { path: entry.path, token: entry.token }
    })
  }
  // return { path: file, token }
  // })
  // if (!interestingFiles.length) return
  // document.interestingFiles = union(document.interestingFiles || [], interestingFiles)
