...
class EntitySpec {
  ...
  static fromUrl(url) {
    ...
    const [, type, provider, namespace, name, revision, toolSpec] = url.match(
      /.*:\/*([^/]+)\/([^/]+)\/([^/]+)\/([^/]+)\/?([^/]+)?(\/tool\/.+)?/
    )
    const [, , toolName, toolVersion] = toolSpec ? toolSpec.split('/') : []
    return new EntitySpec(type, provider, namespace, name, revision, toolName, toolVersion)
  }

  /** @param {Partial<EntitySpec> | null} spec */
  static fromObject(spec) {
    if (!spec) {
      return null
    }
    if (spec.constructor === EntitySpec) {
      return spec
    }
  ...
  constructor(type, provider, namespace, name, revision = null, tool = null, toolVersion = null) {
    ...
    this.toolVersion = toolVersion?.toLowerCase()
  }
