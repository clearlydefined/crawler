...
class VstsBuild {
  ...
  async queueBuild(request, spec) {
    ...
    if (!definitionId) {
      definitionId = (await this.vstsBuild.createDefinition(this._buildDefinition, this.options.project)).id;
    }
    const build = {
      definition: {
        id: definitionId
  ...
  _getCallbackCommand(requestUrl) {
    ...
    return `curl ${this.crawlerRequestUrl} -X POST -d ${JSON.stringify(body)} --header "Content-Type:application/json" --header "X-token:$(authToken)"`; // $(authToken) is a secret variable defined in VSTS
  }
